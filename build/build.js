...
function meta() {
  ...
  const name = pkg.name.replace('@nextgis/', '');
  return { rootPath, pkgPath, pkgDir, pkgFullPath, pkg, name };
}

async function build() {
  const { pkg, pkgDir, rootPath } = meta();
  ...
  if (buildTypes && pkg.types) {
    ...
    const publicTrimmedFilePath = path.resolve(`${meta}/lib/index.d.ts`);
...
function checkSize(target) {
  const { name } = meta();
  checkFileSize(`./lib/${name}.esm-bundler.prod.js`);
}
